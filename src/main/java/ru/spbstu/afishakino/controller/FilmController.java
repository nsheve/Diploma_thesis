...
@Tag(name = "FilmController", description = "Получаем информацию по фильму + разные фильтры")
public class FilmController {
    ...
    @GetMapping("/films")
    @Operation(summary = "Информация о всех фильмах",
            description = "Получаем все информацию о фильме(Жанр, название и тд, кроме сеанса и названия кинотеатре)")
    public ResponseEntity<List<Film>> getAllFilms() {
        return new ResponseEntity<>(filmService.getAllListFilm(), HttpStatus.OK);
    }

    @GetMapping("/filmById/{id}")
    @Operation(summary = "Информация о фильме по id",
            description = "По определенному id получаем информацию о фильме")
    public ResponseEntity findById(@PathVariable long id) {
        return new ResponseEntity(filmService.findFilmId(id), HttpStatus.OK);
    }

    @GetMapping("/filmByTitle/{title}")
    @Operation(summary = "Информация о фильме по названию",
            description = "По определенному названию фильма получаем все информацию о фильме")
    public ResponseEntity findByTitle(@PathVariable String title) {
        return new ResponseEntity(filmService.findFilmTitle(title), HttpStatus.OK);
    }

    @GetMapping("/filmFilterGenre/{genre}")
    @Operation(summary = "Фильтр по жанру",
            description = "Получаем фильмы по определенному жанру")
