...
public class ScheduleController {
    ...
    @Operation(summary = "Получаем всею инфу о расписании фильма",
            description = "Кратко говоря, получаем все информацию о каждом фильме + его сеансу + в каком кинотеаре")
    public ResponseEntity<List<Schedule>> getAllSchedules() {
        return new ResponseEntity<>(scheduleService.getAllScheduleList(), HttpStatus.OK);
    }

    @GetMapping("/schedule/{id}")
    @Operation(summary = "Получаем по id расписание фильма",
            description = "Получаем по id, расписание фильма + кинотеатр + информацию о фильме")
    public ResponseEntity findById(@PathVariable long id) {
        return new ResponseEntity(scheduleService.findSchedule(id), HttpStatus.OK);
    }

    @GetMapping("/films/{day}")
    @Operation(summary = "Фильмы с их сеансами",
            description = "Все фильмы(с их сеансами) в определенный день, день у меня хранится в String(если брать наш день, то дата = сегодня иначе число+месяц)")
    ...
    @Operation(summary = "Получаем сеансы фильма",
            description = "Все сеансы определенного фильма по ID")
